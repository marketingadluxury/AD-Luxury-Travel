...
### 1.16 Tối ưu UI/UX Dashboard: Đồng bộ kích thước View Switchers & Khắc phục lỗi chồng chữ Biểu đồ tròn (PieChart)
- **Mô tả lỗi:** 
  1. Các nút chuyển đổi chế độ xem (Bảng, Cột, Đường, Hình tròn) trên các khối báo cáo của Dashboard (`Team Performance`, `Sale Performance`, `Leaderboard`) có kích thước không đồng bộ, một số tab bị co giãn bẹp hoặc mất viền padding.
  2. Khi chuyển sang chế độ xem Biểu đồ tròn (`PieChart`), nếu các mục dữ liệu bằng 0 (hoặc có tỷ lệ % quá nhỏ < 5%), các nhãn văn bản (labels) hiển thị bị xếp đè lên nhau ở tâm biểu đồ, gây rối mắt và che mất thông tin.
- **Giải pháp:**
